...
const getRecipe = async () => {
  return new Promise((resolve, reject) => {
    ...
    FROM recipe JOIN category ON recipe.category_id = category.id JOIN users ON recipe.users_id = users.id ORDER BY recipe.id DESC;`,
      (err, results) => {
        if (!err) {
          resolve(results);
        } else {
...
const postRecipe = async (post) => {
  return new Promise((resolve, reject) => {
    ...
    Pool.query(
      "INSERT INTO recipe (title, img, ingredients, category_id, users_id, public_id) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *",
      [title, img, ingredients, category_id, users_id, public_id],
      (err, results) => {
        if (!err) {
          resolve(results);
        } else {
...
const getRecipeByUser = async (id) => {
  return new Promise((resolve, reject) => {
    console.log("Model: Get recipe by ID");
    Pool.query(
      `SELECT recipe.id, recipe.title, recipe.img, recipe.ingredients, recipe.public_id, recipe.created_at, category.name AS category, users.name AS author FROM recipe JOIN category ON recipe.category_id = category.id JOIN users ON recipe.users_id = users.id WHERE recipe.users_id = ${id}`,
      (err, results) => {
        if (!err) {
          resolve(results);
        } else {
